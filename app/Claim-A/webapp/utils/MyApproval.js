...
], function (Filter, FilterOperator, Sorter, JSONModel, MessageToast, Utility) {
    "use strict";
    ...
    return {
        ...
        _loadRequestById: async function (oController, sRequestId) {
            ...
            try {
                ...
                const nCashadv_amt = aItems.reduce((sum, it) =>
                    it.CASH_ADVANCE === "YES" ? sum + (Number(it.EST_AMOUNT) || 0) : sum, 0);
                const nReq_amt = aItems.reduce((sum, it) =>
                    it.CASH_ADVANCE === null || it.CASH_ADVANCE === "NO" ? sum + (Number(it.EST_AMOUNT) || 0) : sum, 0);

                oReq.setProperty("/req_header/cashadvamt", nCashadv_amt);
                oReq.setProperty("/req_header/reqamt", nReq_amt);
        ...
        _loadClaimById: async function (oController, sClaimId) {
            const oClaimInputModel = this._getClaimInputModel(oController);
            const oEmployeeModel = await this._ensureModelReady(oController, "employee_view");
            const oODataModel = oController.getOwnerComponent().getModel();

            const aFilters = [new Filter("CLAIM_ID", FilterOperator.EQ, sClaimId)];

            const oHeaderBinding = oEmployeeModel.bindList("/ZEMP_CLAIM_HEADER_VIEW", null, null, aFilters, {
                $$ownRequest: true, $count: true, $select: ["*"]
            });
            const oItemBinding = oODataModel.bindList("/ZCLAIM_ITEM", null,
                [new Sorter("CLAIM_SUB_ID", false)], aFilters, {
                $$ownRequest: true, $count: true, $select: ["*"]
            }
            );
            const oItemDescrBinding = oEmployeeModel.bindList("/ZEMP_CLAIM_ITEM_VIEW", null,
                [new Sorter("CLAIM_SUB_ID", false)], aFilters, {
                $$ownRequest: true, $count: true, $select: ["*"]
            }
            );
            ...
            try {
                ...
                oClaimInputModel.setProperty("/claim_header", this._mapClaimHeaderToForm(oHeaderRaw));
        ...
        _waitForModel: function (oController, sModelName) {
            return new Promise((resolve) => {
                const check = () => {
                    const oModel = oController.getOwnerComponent().getModel(sModelName);
                    if (oModel) resolve(oModel);
        ...
        _ensureModelReady: async function (oController, sModelName) {
            const oModel = await this._waitForModel(oController, sModelName);
