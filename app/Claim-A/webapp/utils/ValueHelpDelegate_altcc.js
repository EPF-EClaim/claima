		...
		return new Promise((fnResolve, fnReject) => {
			sap.ui.require(["sap/m/library", "sap/m/Table", "sap/m/Column", "sap/m/ColumnListItem", "sap/m/Label", "sap/m/Text", "sap/ui/model/type/String"], function() {
				...
				const oTable = new Table(oContainer.getId() + "-Table", {
					...
					items: {path: "employee>/ZCOST_CENTER", template: new ColumnListItem({
						type: "Active",
						cells: [
							new Text({text: {path: 'employee>COST_CENTER_ID', type: new StringType()}}),
							new Text({text: {path: 'employee>COST_CENTER_DESC', type: new StringType()}})
						]
					})}
				});
