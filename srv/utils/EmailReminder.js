...
module.exports = {
    ...
    getFirstScenario: function(sTripEndDate, sTodayDate) {
        const aMilestones = { 1: '1', 30: '30', 60: '60', 85: '85' };
        const nDifference = this.getDateDifference(sTripEndDate, sTodayDate);
        return aMilestones[nDifference] || null;
    },
    ...
    getClaimStatus: async function(ZCLAIM_HEADER, tx, sPreapprovalID) {
        ...
        return claim?.STATUS_ID === 'STAT01' || !claim;
    },

    getClaimantDetails: async function(ZEMP_MASTER, ZROLEHIERARCHY, ZCONSTANT, tx, sEmpID, nScenario) {
    ...
    getZConstant: async function(ZCONSTANT, tx) {
        const aConstant = await tx.run(SELECT.from(ZCONSTANT));
        return aConstant;
    }
};