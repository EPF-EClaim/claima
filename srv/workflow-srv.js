    ...
    srv.on('startWorkflow', async req => {
        ...
        if (aApproversContext.length && aApproversContext[0].LEVEL == 0) {
            try {
                ...
                await oTx.rollback();
                console.error(`[${sId}] Budget actualization failed:`, error);
            ...
            if (oReturn) {
                await oTx.rollback();
                return generateReturnMessage(bStatus, sId, Constant.WorkflowArea.BUDGET_ACTUALIZATION, 'Error encountered during Budget Checking', false);
            }
            ...
            try {
                ...
                await oTx.rollback();
                console.error(`[${sId}] Header update to APPROVED failed:`, error);
                throw new Error(`Error encountered while updating claim status to Approved${error.message}`);
            }
        } else {
            ...
            try {
                ...
                await oTx.rollback();
                console.error(`[${sId}] Corporate Card Advance / header update failed:`, error);
                throw new Error(`Error encountered during Corporate Card Advance update${error.message}`);
            }
        }
        // ==========
        ...
        if (oWorkflowContext && aApproversContext.length) {
            ...
            try {
                ...
                console.error(`[${sId}] Failed to write ZLOG entry:`, error);
            }
            ...
            try {
                if (aApproversContextNew[0].LEVEL == 0) {
                    await sendEmailToClaimant(sId, aApproversContextNew[0].APPROVER_ID, oDescriptor, Constant.ApprovalEmailAction.ACTION_APPROVE);
                    await sendClaimBatch(sId);
                } else {
                    await sendEmailToApprover(aApproversContext, sId, oDescriptor, Constant.ApprovalEmailAction.ACTION_NOTIFY);
                }
            } catch (error) {
                console.error(`[${sId}] Notification step failed:`, error);
            }
        }
        // ==========
